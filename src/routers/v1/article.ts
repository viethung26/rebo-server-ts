...
export default class ArticleRouter extends CrudRouter<ArticleController> {
    ...
    customRouter() {
        ...
        this.router.get("/interest", authenticateMiddleware.run(), getQueryMiddleware.run(), this.route(this.getInterest))
    }
    ...
    async getInterest(req: Request, res: Response) {
        if (req.user) {
            const result = await this.controller.readInterest(req?.user?.categories || [], {})
            return this.onSuccess(res, result)
